...
{
    /// <summary>
    /// 这里要说下，昨天有人问我DbContext.cs内容讲一下，
    /// 其实呢，这个类里，很简单，主要是1、获取SqlSugarClient实例，
    /// 2、CodeFirst（根据实体类生成数据库表）；
    /// 3、DbFirst（根据数据库表生成实体类）。
    /// 只不过都是用到的同名方法重载，可能看上去比较累，可以调用一次就能明白了。

    ///这里简单说下DbFirst吧，其他的可以自行研究下，也可以右侧扫码加QQ群，
    ///我们一对一讨论。DbFirst是一个根据数据库表生成实体类的过程，前提是要有系统表的权限,
    ///否则无法读取表的结构，框架在底层封装了很多模板，将真实值填充进去，特别像是动软代码生成器或者T4模板。
    /// </summary>
